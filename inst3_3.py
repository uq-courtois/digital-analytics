...
for item in linkdiv.find_all('li'):
	...
	# Request information at url location
	req = Request(url, headers=headers)
	context = ssl._create_unverified_context()
	uClient= urlopen(req, context=context)
	html = uClient.read() # html is stored in variable html
	...
	# Interpret the page source as html
	souppostpage = BeautifulSoup(html, 'html.parser')

	# Get postdiv
	blogpost = souppostpage.find('div',class_='blogpost')

	# Print title text
	print(blogpost.find('h2').getText())
