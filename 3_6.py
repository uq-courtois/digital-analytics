...
for row in tabledata.find_all('tr'):
	...
	try:
		album = row.find('td', class_ = 'albumtitle').getText() # CHANGED!
		sales = row.find('td', class_ = 'sale').getText() # CHANGED!
		release = row.find('td', class_ = 'releaseyear').getText() # CHANGED!
